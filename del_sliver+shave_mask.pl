...
$genesis -> COM ("affected_filter",filter=>"(type=solder_mask|components|mask&context=board&side=top|bottom|none&pol=positive)");
...
$genesis -> COM ("set_filter_attributes",filter_name=>"popup",exclude_attributes=>"no",condition=>"no",attribute=>".shave",min_int_val=>"0",max_int_val=>"0",min_float_val=>"0",max_float_val=>"0",option=>"",text=>"");
