...
#-------------------------------------------------------------------------------------------#
# Description: Create mark lines on the production panel and new layer for export scoring file
# Author:RVI
#-------------------------------------------------------------------------------------------#

package Packages::Scoring::ScoreMarkPanel;
...
sub SetScoreOnPanel {
	...
	my $genesis = shift;
	my $jobName = shift;
	my $stepName = EnumsProducPanel->PANEL_NAME;
				  ...
				  $genesis->INFO(entity_type=>'layer',entity_path=>"$jobName/$stepName/v2",data_type=>'exists');
						if ($genesis->{doinfo}{gEXISTS} eq "yes") {
										...
										open (INFOFILE,$infoFile);
												while(<INFOFILE>) {
														if ($_ =~ /\.pnl_place=O-Outer_VV-left-bot/) {
								...
								$odskokY = 18;
										...
										open (INFOFILE,$infoFile);
												while(<INFOFILE>) {
														if ($_ =~ /\.pnl_place=O-Outer_2V-left-bot/) {
																my @infoLine = split /\s/,$_;
																		$drOtvorX = $infoLine[1];
																		$drOtvorY = $infoLine[2];
														}
				...
				my $scoreLayerName = "score";
					...
					$i = 0;
				    	...
				    	$genesis->INFO(entity_type=>'layer',entity_path=>"$jobName/panel/draz_prog",data_type=>'exists');
				    		...
				    		$genesis -> COM ('display_layer',name=>'draz_prog',display=>'no',number=>'1');
