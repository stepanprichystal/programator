...
# Author:SPR
#-------------------------------------------------------------------------------------------#
package Packages::Coupon::CpnSource::CpnSource;
...
#local library
use XML::LibXML;
use aliased 'Packages::Coupon::CpnSource::Constraint';
...
sub GetConstraints {
	...
	foreach my $constraint ( $self->{"dom"}->findnodes('/document/interfacelist/JOB/STACKUP/STACKUP/IMPEDANCE_CONSTRAINTS/IMPEDANCE_CONSTRAINT') ) {
		...
		my $c = Constraint->new( $self, $self->{"units"}, scalar(@constraints), $tInStack, $mInStack, $constraint );
