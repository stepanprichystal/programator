...
sub new {
	...
	# Property for reference filter
	$self->{"refLayers"} = [];            # reference layers

	$self->{"refFilterMode"};             # reference filter mode
	...
	if ( scalar( @{ $self->{"featureIndexes"} } ) ) {
		...
		for ( my $i = 0 ; $i < scalar(@ids) ; $i++ ) {
			...
			if ( scalar(@idsPart) == 20 ) {
				...
				if ( scalar( @{ $self->{"refLayers"} } ) ) {
					$inCAM->COM(
								 'sel_ref_feat',
								 'layers'   => join( "\\;", @{ $self->{"refLayers"} } ),
								 'use'      => 'filter',
								 'mode'     => $self->{"refFilterMode"},
								 'pads_as'  => 'shape',
...
sub SetReferenceLayers {
	my $self      = shift;
	my $mode      = shift;    # touch, disjoint, etc..
	my $refLayers = shift;

	$self->{"refLayers"}     = $refLayers;
	$self->{"refFilterMode"} = $mode;
