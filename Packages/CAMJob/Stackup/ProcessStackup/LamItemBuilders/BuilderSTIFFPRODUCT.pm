...
sub Build {
	...
	my $item = $lam->AddItem( "product",
							  Enums->ItemType_MATPRODUCT,
							  EnumsStyle->GetItemTitle( Enums->ItemType_MATPRODUCT ),$product, undef, undef, $stckpMngr->GetThick( 1, 0 ) );

	$lam->AddChildItem( $item, "top", "productTop", Enums->ItemType_MATPRODUCT, "TOP", undef, undef, undef, undef );
	$lam->AddChildItem( $item, "bot", "productBot", Enums->ItemType_MATPRODUCT, "BOT", undef, undef, undef, undef );
