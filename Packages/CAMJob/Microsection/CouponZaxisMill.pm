...
my $DEF_STEP_PLUSCNT  = 3;     # three coupons with greater depth than computed (increased by step size)
my $DEF_STEP_MINUSCNT = 3;     # three coupons with smaller depth than computed (decreased by step size)
...
sub GetAllSpecifications {
	...
	foreach my $l (@depthLayers) {
		...
		$specInf{"measureValue"} = defined $pcbThickValue ? $pcbThickValue : $depthValue;                        # in [um]
...
if ( $filename =~ /DEBUG_FILE.pl/ ) {
	...
	my @spec = $c->GetAllSpecifications();
