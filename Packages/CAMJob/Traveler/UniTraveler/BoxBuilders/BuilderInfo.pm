...
# Author:SPR
#-------------------------------------------------------------------------------------------#
package Packages::CAMJob::Traveler::UniTraveler::BoxBuilders::BuilderInfo;
...
sub __BuildMatInfo {
	...
	my $pcbType = $travelMngr->GetISPcbType();
	...
	my $cuStr = $travelMngr->GetCuLayerCnt();
	...
	# Kinds
	my @mats = $travelMngr->GetBaseMaterialInfo();
	...
	my ( $w, $h ) = $travelMngr->GetPanelSize();
...
sub __BuildPaketInfo {
	...
	my $matThick    = $traveler->GetPaketThick(0);
	...
	my $paketThick    = $traveler->GetPaketThick(1);
	...
	my @outerPads = $traveler->GetOuterPresspads(); # outer pads (top/bot), which are outer of inner steel plates
	if ( scalar(@outerPads) ) {
...
sub __BuildOperInfo {
	...
	my @allLam = $travelMngr->GetAllLamination();
	...
	for ( my $i = 0 ; $i < scalar(@allLam) ; $i++ ) {
		...
		if ( $curLam->GetLamOrder() eq $traveler->GetLamOrder() ) {
	...
	$tbl->AddRowDef( $tbl->GetRowCnt(), EnumsStyle->RowHeight_STD );
	my $strOrder = ( $traveler->GetLamOrder() + 1 ) . "/" . scalar(@allLam);
	...
	$tbl->AddCell( $tbl->GetCollDefPos( $tbl->GetCollByKey("rightCol") ),
				   $tbl->GetRowCnt() - 1,
				   undef, undef, $traveler->GetProductId(),
				   $txtRCollStyle );
...
sub __BuildLamInfo {
	...
	my %pInfo = $travelMngr->GetPressProgramInfo( $traveler->GetLamType(), $traveler->GetLamData() );
...
sub __BuildNoteInfo {
	...
	my @noflow = map { $_->GetValExtraId() } grep { $_->GetItemType() eq Enums->ItemType_MATFLEXPREPREG } $traveler->GetItems();
