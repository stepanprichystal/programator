...
# Author:SPR
#-------------------------------------------------------------------------------------------#
package Packages::CAMJob::Traveler::UniTravelerTmpl::BoxBuilders::BuilderTitle;
...
sub Build {
	...
	my $txtRedStyle = TextStyle->new( TblDrawEnums->TextStyle_LINE,
									  EnumsStyle->TxtSize_SMALL, Color->new( 204, 0, 0 ),
									  TblDrawEnums->Font_NORMAL, undef,
									  TblDrawEnums->TextHAlign_RIGHT,
									  TblDrawEnums->TextVAlign_CENTER, 1 );
	...
	# Pcb number
	my $orderId = uc( $stackupMngr->GetOrderId() );

	if ( defined $traveler->GetTravelerType() ) {
		$orderId .= "-" . EnumsStyle->GetTravelerMarking( $traveler->GetTravelerType() );
		$orderId .= $traveler->GetTravelerOrder();

	}

	$tbl->AddCell( $tbl->GetCollDefPos( $tbl->GetCollByKey("col1Text") ), 0, undef, undef, "Císlo zakázky:", $txtStyle );
	$tbl->AddCell( $tbl->GetCollDefPos( $tbl->GetCollByKey("col1Val") ),  0, undef, undef, $orderId,           $txtStylePcbId );
	...
	# Date control
	$tbl->AddCell( $tbl->GetCollDefPos( $tbl->GetCollByKey("col2Val") ),
				   2, undef, undef, "Datumy jsou předběžné, ověř správnost na hlavním postupu!", $txtRedStyle );
