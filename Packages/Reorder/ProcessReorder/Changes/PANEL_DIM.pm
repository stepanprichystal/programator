...
# Author:SPR
#-------------------------------------------------------------------------------------------#
package Packages::Reorder::ProcessReorder::Changes::PANEL_DIM;
...
sub Run {
	...
	my $isInside = 1;
			 ...
			 || $limActive{"yMin"}  > $limSR{"yMin"} )
		{
			...
			$result = 0;
			$$mess .=  "SR steps are placed behind panel active area. Verify that technical schema doesn't overlap steps in panel";
