...
sub __ExportXml {
	...
	$templ->{"job_layer"}->[0]->{"process_parameters"}->[0]->{"registration"}->[0]->{"scale_preset_global"}->[0]->{"x"} = $stretchXVal;
	...
	$templ->{"job_layer"}->[0]->{"display_name"}->[0] = $jobId . $fileName . "_mdi";
	...
	$templ->{"job_layer"}->[0]->{"process_parameters"}->[0]->{"parts_total"}->[0]     = $parts;
	$templ->{"job_layer"}->[0]->{"process_parameters"}->[0]->{"parts_remaining"}->[0] = $parts;
	$templ->{"job_layer"}->[0]->{"process_parameters"}->[0]->{"part_thickness"}->[0]->{"content"} = $self->__GetThickByLayer( $layerName, $etching );;

	#$templ->{"job_layer"}->[0]->{"creation"}->[0]->{"image_size "}->[0]->{"z"} = $self->__GetThickByLayer( $layerName, $etching );
	$templ->{"job_layer"}->[0]->{"creation"}->[0]->{"image_size"}->[0]->{"x"} = $pnlDim->{"w"};
	$templ->{"job_layer"}->[0]->{"creation"}->[0]->{"image_size"}->[0]->{"y"} = $pnlDim->{"h"};

	#$templ->{"job_layer"}->[0]->{"image_position"}->[0]->{"x"} = $x_position;
	#$templ->{"job_layer"}->[0]->{"image_position"}->[0]->{"y"} = $y_position;

	my $mirrorY = $mirror == 1 ? 0 : 1;

	if ($mirrorY) {
		if ( $pnlDim->{"h"} > 540 ) {
			$templ->{"job_layer"}->[0]->{"creation"}->[0]->{"rotation_cw"}->[0]->{"content"} = 270;
		}
		else {
			$templ->{"job_layer"}->[0]->{"creation"}->[0]->{"rotation_cw"}->[0]->{"content"} = 0;
		}
	}
	else {
		if ( $pnlDim->{"h"} > 540 ) {
			...
			$templ->{"job_layer"}->[0]->{"creation"}->[0]->{"rotation_cw"}->[0]->{"content"} = 180;
		}
	}

	$templ->{"job_layer"}->[0]->{"process_parameters"}->[0]->{"registration"}->[0]->{"scaling_mode_global"}->[0]        = $scalingMode;
	$templ->{"job_layer"}->[0]->{"process_parameters"}->[0]->{"registration"}->[0]->{"scale_preset_global"}->[0]->{"x"} = $stretchXVal;
	$templ->{"job_layer"}->[0]->{"process_parameters"}->[0]->{"registration"}->[0]->{"scale_preset_global"}->[0]->{"y"} = $stretchYVal;

	$templ->{"job_layer"}->[0]->{"creation"}->[0]->{"mirror"}->[0]->{"x"} = 0;
	$templ->{"job_layer"}->[0]->{"creation"}->[0]->{"mirror"}->[0]->{"y"} = $mirrorY;

#	$templ->{"job_layer"}->[0]->{"image_object_default"}->[0]->{"image_object"}->[0]->{"diameter_x"}->[0]->{"iterations"}   = $iterations;
#	$templ->{"job_layer"}->[0]->{"image_object_default"}->[0]->{"image_object"}->[0]->{"diameter_x"}->[0]->{"lowch"}        = $upper;
#	$templ->{"job_layer"}->[0]->{"image_object_default"}->[0]->{"image_object"}->[0]->{"diameter_x"}->[0]->{"uppch"}        = $lower;
#	$templ->{"job_layer"}->[0]->{"image_object_default"}->[0]->{"image_object"}->[0]->{"diameter_x"}->[0]->{"value"}        = $diameter;
#	$templ->{"job_layer"}->[0]->{"image_object_default"}->[0]->{"image_object"}->[0]->{"diameter_x"}->[0]->{"upptol"}       = $upperlimit;
#	$templ->{"job_layer"}->[0]->{"image_object_default"}->[0]->{"image_object"}->[0]->{"diameter_x"}->[0]->{"lowtol"}       = $lowerlimit;
#	$templ->{"job_layer"}->[0]->{"image_object_default"}->[0]->{"image_object"}->[0]->{"image_recognition_acceptance"}->[0] = $acceptance;
#	$templ->{"job_layer"}->[0]->{"image_object_default"}->[0]->{"image_object"}->[0]->{"image_acquisition_brightness"}->[0] = $brightness;

	if ( $polarity eq 'negative' ) {
		$templ->{"job_layer"}->[0]->{"creation"}->[0]->{"polarity"}->[0] = 0;
	}
	else {
		$templ->{"job_layer"}->[0]->{"creation"}->[0]->{"polarity"}->[0] = 1;
	}
	if ($power) {
		$templ->{"job_layer"}->[0]->{"process_parameters"}->[0]->{"exposure"}->{"exposure_energy"}->[0]->{"content"} = $power;
	}
	
	$templ->{"job_layer"}->[0]->{"process_parameters"}->[0]->{"fiducial_groups"}->[0]->{"fiducial_group"}->[0]->{"fiducial_id_global"}->[0] = $fiducDCode;
	...
	my $finalFile = EnumsPaths->Jobs_MDITT . $self->{"jobId"} . $fileName . "_mdi.xml";
...
sub __LoadTemplate {
	...
	my $templPath = GeneralHelper->Root() . "\\Packages\\Gerbers\\Mditt\\ExportFiles\\templatett.xml";
