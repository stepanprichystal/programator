...
sub Build {
	...
	for ( my $i = 0 ; $i < $coreCnt ; $i++ ) {
		...
		my $existDrill = $core->ExistNCLayers( Enums->SignalLayer_TOP, EnumsGeneral->LAYERTYPE_nplt_jbMillTop );
	...
	for ( my $i = 0 ; $i < $coreCnt ; $i++ ) {
		...
		my $core = $stackupNC->GetCore($coreNum);
		my $existDrill = $core->ExistNCLayers( Enums->SignalLayer_BOT, EnumsGeneral->LAYERTYPE_nplt_jbMillBot );
