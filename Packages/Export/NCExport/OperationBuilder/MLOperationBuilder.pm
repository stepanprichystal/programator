...
sub __DefinePlatedOperations {
	...
	for ( my $i = 0 ; $i < $coreCnt ; $i++ ) {
		...
		my @ncLayers = ();
		if ( $coreNC->ExistNCLayers( StackEnums->SignalLayer_TOP ) ) {
			push( @ncLayers, $coreNC->GetNCLayers( StackEnums->SignalLayer_TOP ) );
		}
		if ( $coreNC->ExistNCLayers( StackEnums->SignalLayer_BOT ) ) {
			push( @ncLayers, $coreNC->GetNCLayers( StackEnums->SignalLayer_BOT ) );
		}
