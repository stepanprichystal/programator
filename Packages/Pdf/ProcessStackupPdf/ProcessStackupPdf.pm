...
sub new {
	...
	$self->{"paperMargin"} = shift // 10;                              # margin of printer
...
sub BuildTemplate {
	...
	$self->{"processStckp"} = ProcessStackup->new( $inCAM, $self->{"jobId"} );
...
if ( $filename =~ /DEBUG_FILE.pl/ ) {
	...
	my $procStack = ProcessStackup->new( $inCAM, $jobId );
	...
	if ( $procStack->LamintaionCnt() ) {
		...
		if ($resultCreate) {
			...
			$stackup->OutputSerialized( $ser, $lamType );
