...
#-------------------------------------------------------------------------------------------#
# Description: Modul is responsible for creating peelable stencil traveler
# Author:SPR
#-------------------------------------------------------------------------------------------#
package Packages::Pdf::TravelerPdf::PeelStencilPdf::PeelStencilPdf;
...
use aliased 'Packages::Pdf::TravelerPdf::PeelStencilPdf::PeelStencilBuilder';
...
sub BuildTemplate {
	...
	if ( scalar(@NClayers) ) {
		...
		foreach my $NC (@NClayers) {
			push( @bldrs, PeelStencilBuilder->new( $inCAM, $jobId, $NC->{"gROWname"} ) );
		}
...
if ( $filename =~ /DEBUG_FILE.pl/ ) {
	...
	my @stakupTepl = FileHelper->GetFilesNameByPattern( $pDirTraveler, "peel" );
	unlink($_) foreach (@stakupTepl);

	my @stakupPdf = FileHelper->GetFilesNameByPattern( $pDirPdf, "peel" );
	...
	if (1) {
		...
		#my $lamType = ProcStckpEnums->LamType_PeelPRODUCT;
		my $lamType      = undef;
		my $ser          = "";
