...
$sqlStatement = "SELECT 
				lcs.nf_edit_style('stav_zakazky_dps_22', stav) from lcs.zakazky_dps_22_hlavicka WHERE reference_subjektu = '$jobNameNoris'
";
	$sqlExecute = $dbConnection->Execute("$sqlStatement");

		my $resultValue = convert_from_czech ($sqlExecute->Fields("$getValue")->Value);
		$dbConnection->Close();

return ($resultValue);
}
sub GetTerminOfOrder {
		#avoid when package name is as first parameter
		if(@_[0] eq __PACKAGE__) {shift;}; 
...
$sqlStatement = "SELECT termin from lcs.zakazky_dps_22_hlavicka WHERE reference_subjektu = '$jobNameNoris'
";
	$sqlExecute = $dbConnection->Execute("$sqlStatement");

		my $resultValue = convert_from_czech ($sqlExecute->Fields("termin")->Value);
		$dbConnection->Close();
...
$sqlStatement = "SELECT TOP 1 
		case when vs1.cislo_vztaz_subjektu is not null then 'S' when vs2.cislo_subjektu is not null then 'M' else '0' end
		...
		WHERE z.reference_subjektu = '$jobNameNoris'
";
	$sqlExecute = $dbConnection->Execute("$sqlStatement");
