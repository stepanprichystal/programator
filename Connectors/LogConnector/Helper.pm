...
# Author:SPR
#-------------------------------------------------------------------------------------------#
package Connectors::LogConnector::Helper;
...
sub __OpenConnection {
 ...
 #con = DBI->connect( "DBI:mysql:database=$__dbName;
	...
	if ($@) {
		...
		#print STDERR EnumsErrors->HELIOSDBCONN . $@;
		ErrorHandler->LogDatabase(EnumsErrors->LOGDBCONN."\n".$pom);
...
sub ExecuteNonQuery {
	...
	my $con = undef;
	my $cmd = Connectors::LogConnector::Helper->__PrepareCommand( $commandText, $commandParameters );

	$con = Connectors::LogConnector::Helper->__OpenConnection();
	unless ($con) {
		return 0;
	}

	try {
		Connectors::LogConnector::Helper->__Execute( $con, $cmd, 0 );
	}
	catch {

			ErrorHandler->LogDatabase(EnumsErrors->LOGDBERROR . $_);
...
sub ExecuteDataSet {
	...
	if ($con) {
		...
		try {
			@dataset = Connectors::LogConnector::Helper->__Execute( $con, $cmd, 1 );
		}
		catch {
