...
my $main = $inputWindow->Frame(-width=>100, -height=>80)->pack(-side=>'top', -fill=>'both');
		...
		my $mainSubFrameTop = $main->Frame(-width=>100, -height=>80)->pack(-side=>'top', -fill=>'both');
					...
					my $mainSubFrameTop_Left = $mainSubFrameTop->Frame(-width=>100, -height=>80)->pack(-side=>'left', -fill=>'both',-expand => "True");
				 				   	...
				 				   	my $tgzBttn = $mainSubFrameTop_Left->Button(-text => "Open *.tgz|*.zip ...",-font=>'normal 9 {bold }', -command => sub {MultiCustomer('open')});
					...
					my $mainSubFrameBot_Right = $mainSubFrameTop->Frame(-width=>100, -height=>80)->pack(-side=>'right', -fill=>'both',-expand => "True");
									...
									my $mainSubFrameBot_Right_Altium = $mainSubFrameBot_Right->Frame(-width=>100, -height=>80)->pack(-side=>'bottom', -fill=>'both',-expand => "True");
									
													my $buttonFrameTop = $mainSubFrameBot_Right_Altium->Frame(-width=>100, -height=>80)->pack(-side=>'top', -fill=>'both',-expand => "True");
																$buttonFrameTop->Checkbutton(-text=>"Altium",-variable=>\$customerPOOL, -onvalue=>'altium')->pack(-side=>'left', -fill=>'both',-pady=>10);
																
									my $mainSubFrameBot_Right_Pool = $mainSubFrameBot_Right->Frame(-width=>100, -height=>80)->pack(-side=>'top', -fill=>'both',-expand => "True");
									
													my $buttonFrameBot = $mainSubFrameBot_Right_Altium->Frame(-width=>100, -height=>80)->pack(-side=>'bottom', -fill=>'both',-expand => "True");
																$buttonFrameBot->Checkbutton(-text=>"POOL data",-variable=>\$customerPOOL, -onvalue=>'pool')->pack(-side=>'left', -fill=>'both',-pady=>10);
		...
		my $mainSubFrameBot = $main->Frame(-width=>100, -height=>80)->pack(-side=>'bottom', -fill=>'both');
					...
					my $pathFrame = $mainSubFrameBot->Frame(-width=>100, -height=>80)->pack(-side=>'top', -fill=>'both');
											$pathFrameLab = $pathFrame->LabFrame(
														...
														$pathFrameLab->Radiobutton(-value=>'c:/pcb', -variable=>\$pathDisk, -text=>"c:/pcb")->grid(-column=>3,-row=>2,-sticky=>"news",-columnspan=>1);
														$pathFrameLab->Radiobutton(-value=>'r:/pcb', -variable=>\$pathDisk, -text=>"r:/pcb")->grid(-column=>4,-row=>2,-sticky=>"news",-columnspan=>1);
...
#print $importPath;
	...
	$inCAM -> COM('script_run',name=>"//incam/incam_server/site_data/scripts/renameLayerScript.pl",dirmode=>'global',params=>"$jobName $inputStep");
   					...
   					while(){
   						$inCAM->INFO(entity_type=>'layer',entity_path=>"$jobName/o/o",data_type=>'exists');
 							if ($inCAM->{doinfo}{gEXISTS} eq "yes") {
									    ...
									    $inCAM->COM('filter_area_end',layer=>'o',filter_name=>'popup',operation=>'select',area_type=>'none',inside_area=>'no',intersect_area=>'no',lines_only=>'no',ovals_only=>'no',min_len=>'0',max_len=>'0',min_angle=>'0',max_angle=>'0');
					    ...
					    $inCAM->COM('copy_entity',type=>'step',source_job=>"$jobName",source_name=>'o',dest_job=>"$jobName",dest_name=>'o+1',dest_database=>'');
...
# 			OnlineWrite_order( $reference, "zpracovava $hostName" , "aktualni_krok" );
		...
		$inCAM->COM('copy_entity',type=>'step',source_job=>"$jobName",source_name=>"$inputStep",dest_job=>"$jobName",dest_name=>'o+1',dest_database=>'');
		$inCAM->COM ('set_step',name=>"$inputStep");
 			 	...
 			 	$inCAM->INFO(entity_type=>'step',entity_path=>"$pcbId/mpanel",data_type=>'exists');
						if ($inCAM->{doinfo}{gEXISTS} eq "yes") {
								if (HegMethods->GetPcbIsPool($pcbId) == 1) {
												...
												$inCAM -> COM('copy_entity',type=>'step',source_job=>"$pcbId",source_name=>'mpanel',dest_job=>"$pcbId",dest_name=>"$stepNetlist",dest_database=>'');
												
												$inCAM -> COM ('set_step',name=>'mpanel');
												$inCAM -> COM ('script_run',name=>"//incam/incam_server/site_data/scripts/flatten_pool.pl",dirmode=>'global',params=>"$pcbId mpanel");
...
sub _SearchTgz {
		...
		my @listFiles = ();
					...
					unless (scalar @tgzArr == 1) {
								my @mess = ("Nemùžu najít správný tgz file pro import!");
								new MessageForm( Enums::MessageType->INFORMATION, \@mess, undef );
								exit;
					}else{
							return($tgzArr[0]);
...
sub _NetlistCompare {
		...
		my $stepId = shift;
			...
			$inCAM -> COM ('netlist_compare',job2=>"$jobId",step2=>"$stepId",type2=>'cur',recalc_cur=>'no',use_cad_names=>'no',report_extra=>'yes',report_miss_on_cu=>'yes',report_miss=>'yes',max_highlight_shapes=>'5000');
...
sub _MoveToZeroPoint {
		...
		if ($zeroPointX != 0 or $zeroPointY != 0) {
		    	...
		    	$inCAM->COM('work_layer',name=>"$tmpLayer");
	   			$inCAM->COM('filter_area_strt');
	   			$inCAM->COM('filter_area_end',layer=>"$tmpLayer",filter_name=>'popup',operation=>'select',area_type=>'none',inside_area=>'no',intersect_area=>'no',lines_only=>'no',ovals_only=>'no',min_len=>'0',max_len=>'0',min_angle=>'0',max_angle=>'0');
	   			$inCAM->COM('sel_create_profile');
	   			$inCAM->COM('filter_reset',filter_name=>'popup');
...
sub _MoveNonPlateHoles {
		...
		$inCAM->COM ('filter_area_strt');
		$inCAM->COM ('filter_area_end',layer=>'',filter_name=>'popup',operation=>'select',area_type=>'none',inside_area=>'no',intersect_area=>'no',lines_only=>'no',ovals_only=>'no',min_len=>'0',max_len=>'0',min_angle=>'0',max_angle=>'0');
		...
		$inCAM->COM ('sel_ref_feat',layers=>'',use=>'select',mode=>'disjoint',f_types=>'line\;pad\;surface\;arc\;text',polarity=>'positive\;negative',include_syms=>'',exclude_syms=>'');
		$inCAM->COM ('get_select_count');
			if ($inCAM->{COMANS} == 0) {
        			$inCAM->COM('filter_reset',filter_name=>'popup');
					$inCAM->COM('affected_layer',name=>"",mode=>"all",affected=>"no");
...
sub _SelectInputStep {
		...
		my $returnStep;
  			...
  			my $exitBttn = $mainStep->Button(-text => "Pokracovat", -command =>\sub{$returnStep = $stepWin->get('active');$mainStep->destroy;})->pack(-side=>'top', -fill=>'both');
...
sub _ChangeMinLineRout {
		...
		my $layerRout = 'f';
				...
				my $infoFile = $inCAM->INFO('units'=>'mm','entity_type'=>'layer','entity_path'=>"$pcbId/$stepName/$layerRout",'data_type'=>'FEATURES',parse=>'no');
