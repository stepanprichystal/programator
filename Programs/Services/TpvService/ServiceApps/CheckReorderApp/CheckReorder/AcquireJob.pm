...
# return 0, if job in InCAM doesnt exist (plus store err mess to $errMess ref)
	...
	unless ( CamJob->JobExist( $inCAM, $jobId ) ) {
		...
		if ( -e $path ) {
			...
			if ($importSucc) {
				...
				$$errMess .= "Error during import job to InCAM db. $importErr" if ( defined $errMess );
			...
			if ( defined $errMess ) {
				...
				$$errMess .= "Error during import job to InCAM db. TGZ file doesn't exist at $path ";
				$$errMess .= "(PcbId: $jobId => former Pcbid: $oldPcbId) " if ( defined $oldPcbId );
			}
		}
	}
...
sub Acquire2 {
	...
	my $result = $self->Acquire( $inCAM, $jobId, $errMess);
