...
sub OnCheckGroupData {
	...
	if ( abs( $prof{"yMax"} - $prof{"yMin"} ) == 460 && $defaultInfo->GetBaseCuThick() == 18 && $defaultInfo->GetPcbThick() == 1500 ) {

		$dataMngr->_AddErrorResult( "Pozor, materiálů nelze použít",
									"Pozor nelze použít přířez 295*460*1,5mm 18/18 informace od vedouciho. Použij nový velký panel" );
