...
sub __SetLayout {
	...
	my $layerColor = LayerColorPnl->new( $self, $layerName, 10 );

	my $topLayerTxt =
	  Wx::StaticText->new( $self, -1, ( defined $self->GetTopLayer() ? $self->GetTopLayer() : "-" ), &Wx::wxDefaultPosition, [ 50, 22 ] );
	my $botLayerTxt =
	  Wx::StaticText->new( $self, -1, ( defined $self->GetBotLayer() ? $self->GetBotLayer() : "-" ), &Wx::wxDefaultPosition, [ 50, 22 ] );

	my @rotOptions = ( "V", "H" );

	my $topLayerRotChb = Wx::ComboBox->new( $self, -1, "", &Wx::wxDefaultPosition, [ 32, 22 ], \@rotOptions, &Wx::wxCB_READONLY );
	my $botLayerRotChb = Wx::ComboBox->new( $self, -1, "", &Wx::wxDefaultPosition, [ 32, 22 ], \@rotOptions, &Wx::wxCB_READONLY );
	...
	my $topLayerFiducChb = Wx::ComboBox->new( $self, -1, "", &Wx::wxDefaultPosition, [ 10, 22 ], \@fiducOptions, &Wx::wxCB_READONLY );
	my $botLayerFiducChb = Wx::ComboBox->new( $self, -1, "", &Wx::wxDefaultPosition, [ 10, 22 ], \@fiducOptions, &Wx::wxCB_READONLY );

	if ( !defined $self->GetTopLayer() ) {
		$topLayerRotChb->Disable();
		$topLayerFiducChb->Disable();
	}

	if ( !defined $self->GetBotLayer() ) {
		$botLayerRotChb->Disable();
		$botLayerFiducChb->Disable();
	}
