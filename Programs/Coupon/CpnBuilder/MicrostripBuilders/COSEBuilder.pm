...
#-------------------------------------------------------------------------------------------#
# Description: Coplanar single ended builder
# Author:SPR
#-------------------------------------------------------------------------------------------#
...
sub Build {
	...
	# 2) Set GND to track/pads distance
	...
	if ( $cpnSett->GetGNDViaShielding() ) {
		...
		my $yPosTopVia =
		  $tOrigin->Y() +
		  ( $stripVariant->RouteWidth() / 2 * 1000 + $coSE + $cpnSett->GetGNDViaHole2GNDDist() + $cpnSett->GetGNDViaHoleSize() / 2 ) / 1000;
		my $yPosBotVia =
		  $tOrigin->Y() -
		  ( $stripVariant->RouteWidth() / 2 * 1000 + $coSE + $cpnSett->GetGNDViaHole2GNDDist() + $cpnSett->GetGNDViaHoleSize() / 2 ) / 1000;
