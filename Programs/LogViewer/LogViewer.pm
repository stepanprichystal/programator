...
sub __SetLayout {
	...
	#Wx::Event::EVT_CLOSE($mainFrm, sub{__OnClose($self,@_) } );
	...
	#$imagelist->Add( $btm, &Wx::wxNullBitmap);

	#$nb->SetPageImage(0,0);

	#$nb->SetPadding([20,20]);

	$self->__SetLayoutActions($nb);
	$self->__SetLayoutMDI($nb);
	$self->__SetLayoutJetPrint($nb);

	#

	#print $listCtrl;

	#$mainFrm->SetMinSize( $mainFrm->GetSize() );
...
sub __SetLayoutMDI {
	...
	my $self        = shift;
	my $nb          = shift;
	my $pageMainPnl = $self->__AddPage( $nb, "MDI" );

	$self->{"mdiPnl"}  = $pageMainPnl;
	$self->{"mdiList"} = $pageMainPnl;
}
...
sub __SetLayoutJetPrint {
	...
	my $self        = shift;
	my $nb          = shift;
	my $pageMainPnl = $self->__AddPage( $nb, "Jet Print" );
